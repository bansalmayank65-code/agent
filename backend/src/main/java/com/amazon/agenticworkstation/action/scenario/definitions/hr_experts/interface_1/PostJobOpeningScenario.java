...
/**
 * Post Job Opening Scenario.
 * 
 * This scenario provides a complete workflow for posting job openings in the HR
 * system.
 * 
 * Workflow: 1. Discover job position 2. Update job position status to open 3.
 * Create audit log entry
 * 
 * Category: Job Position Management Environment: hr_experts Interface: 1
 */
public final class PostJobOpeningScenario implements BaseScenario {
	private static final Logger log = LoggerFactory.getLogger(PostJobOpeningScenario.class);
	...
	public PostJobOpeningScenario() {
		this.parameters = new LinkedHashMap<>();
	}

	public PostJobOpeningScenario(Map<String, Object> parameters) {
		if (parameters == null) {
	...
	public Map<String, Object> getScenarioInputs() {
		...
		scenarioInputs.put("position_id_filter",
				String.format("{\"position_id\":\"%s\",\"status\":\"draft\"}", positionId));

		return scenarioInputs;
	}

	private static ScenarioConfig buildScenarioConfig() {
		log.debug("Building post_job_opening scenario configuration");

		return ScenarioConfig.builder().scenarioName("post_job_opening")
				.description("Complete workflow for posting job openings").envName("hr_experts").interfaceNumber(1)
				.steps(Arrays.asList(ScenarioStep.builder().stepId("step1_discover_user")
						...
						ScenarioStep.builder().stepId("step3_update_position_status").actionName("manage_job_position")
								...
								.addInputMapping(InputMapping.withStaticValue("status", "open"))
								.addInputMapping(InputMapping.withStaticValue("action", "update"))
								.description("Update job position status to open").build(),
						...
						ScenarioStep.builder().stepId("step4_audit_log").actionName("manage_audit_logs")
								...
								.addInputMapping(InputMapping.withStaticValue("action", "update"))
								.addInputMapping(InputMapping.withStaticValue("operation", "post"))
								.addInputMapping(InputMapping.withStaticValue("reference_type", "job_positions"))
								.description("Create audit log for job posting").build()))
				.build();
	}
	...
	private static List<ScenarioInputDefinition> buildRequiredInputs() {
		return Arrays.asList(ScenarioInputDefinition.builder().name("position_id").type("string")
				.description("ID of the job position to post").required(true).example("12345").build());
	}
}
